...
const SIZE_MEMORY = SIZE_STATE + SIZE_KEY + SIZE_SHIFT_TABLE + SIZE_SBOX_TABLE + SIZE_SBOX_INV_TABLE + SIZE_XOR_TABLE + SIZE_AND_TABLE * 8 + SIZE_SBOX_XOR_TABLES + SIZE_SBOX_INV_XOR_TABLES
...
function scriptNumPushCost(n) {
    if (n === 0) return 1;                         // OP_0
    if (n === -1 || (n >= 1 && n <= 16)) return 1; // OP_1NEGATE / OP_1..OP_16
...
function rollCost(pos) {
    if (pos === 0) return 0;
    if (pos === 1) return 1; // OP_SWAP
    if (pos === 2) return 1; // OP_ROT
    return scriptNumPushCost(pos) + 1; // <pos> OP_ROLL
}

function xorShiftedCost(scratch) {
    if (scratch === 0) return 2;                      // OP_ADD OP_PICK
    if (scratch === 1 || scratch === -1) return 3;    // OP_ADD OP_1ADD/OP_1SUB OP_PICK
    return 3 + scriptNumPushCost(Math.abs(scratch));  // OP_ADD <k> OP_ADD/SUB OP_PICK
}
...
function mhatCoreCostForPerm(perm, rot, scratch = 0) {
    ...
    for (let r = 0; r < 4; r++) {
        ...
        cost += xorShiftedCost(s - 1 - AND_VALUE_BIAS - XOR_SHIFT_BIAS[c1]);
        cost += xorShiftedCost(s - 2 - XOR_SHIFT_BIAS[c2]);
        cost += xorShiftedCost(s - 3 - XOR_SHIFT_BIAS[c3]);
    }
...
function bestMHatPermForEnv(env, stateIndices, rot, scratch = 0) {
    ...
    return { perm: bestPerm, cost: bestCost };
}
...
const ADDR_AND_M2_SHIFT_TABLE = 64
const ADDR_AND_M3_SHIFT_TABLE = 80
const ADDR_SBOX_INV_TABLE = 128
const ADDR_SHIFT_TABLE = 144
const ADDR_AND_M0_TABLE = 672
const ADDR_AND_M1_TABLE = 512
const ADDR_AND_M2_TABLE = 560
const ADDR_AND_M3_TABLE = 640
const ADDR_XOR_TABLE = 176
const ADDR_SBOX_XOR_2_TABLE = 432
const ADDR_SBOX_XOR_3_TABLE = 624
const ADDR_SBOX_XOR_4_TABLE = 496
const ADDR_SBOX_XOR_8_TABLE = 480
const ADDR_SBOX_XOR_9_TABLE = 592
const ADDR_SBOX_XOR_14_TABLE = 448
const ADDR_SBOX_INV_XOR_3_TABLE = 528
const ADDR_SBOX_INV_XOR_5_TABLE = 656
const ADDR_SBOX_INV_XOR_7_TABLE = 160
const ADDR_SBOX_INV_XOR_8_TABLE = 608
const ADDR_SBOX_INV_XOR_9_TABLE = 544
const ADDR_SBOX_INV_XOR_12_TABLE = 464
const ADDR_SBOX_INV_XOR_13_TABLE = 576
const AND_VALUE_BIAS = 6
const XOR_SHIFT_BIAS = [1, 1, 5, -3]

// XOR rows are permuted in push order to make the static table cheaper to construct.
// XOR_ROW_ORDER is deepest row first, top row last.
const XOR_ROW_ORDER = [15, 3, 13, 1, 11, 7, 9, 5, 10, 6, 8, 4, 14, 2, 12, 0]
const XOR_ROW_OFFSET = (() => {
    const out = Array(16);
    XOR_ROW_ORDER.forEach((row, pushIndex) => { out[row] = 16 * (15 - pushIndex); });
    return out;
})()


const push_xor_table = XOR_ROW_ORDER.map(row => loop(16, j => row ^ (15 - j)))
const push_shift_table = loop(SIZE_SHIFT_TABLE, i => XOR_ROW_OFFSET[i] + ADDR_XOR_TABLE - 1).reverse()
...
const push_and_shift_table_m0 = loop(16, i => XOR_ROW_OFFSET[M[0] & i] + ADDR_XOR_TABLE - 1 + XOR_SHIFT_BIAS[0]).reverse()
const push_and_shift_table_m1 = loop(16, i => XOR_ROW_OFFSET[M[1] & i] + ADDR_XOR_TABLE - 1 + XOR_SHIFT_BIAS[1]).reverse()
const push_and_shift_table_m2 = loop(16, i => XOR_ROW_OFFSET[M[2] & i] + ADDR_XOR_TABLE - 1 + XOR_SHIFT_BIAS[2]).reverse()
const push_and_shift_table_m3 = loop(16, i => XOR_ROW_OFFSET[M[3] & i] + ADDR_XOR_TABLE - 1 + XOR_SHIFT_BIAS[3]).reverse()
...
const push_sbox_inv_xor_3_table  = loop(16, i => PRINCE_SBOX_INVERSE[i] ^ 3).reverse()
const push_sbox_inv_xor_5_table  = loop(16, i => PRINCE_SBOX_INVERSE[i] ^ 5).reverse()
const push_sbox_inv_xor_7_table  = loop(16, i => PRINCE_SBOX_INVERSE[i] ^ 7).reverse()
const push_sbox_inv_xor_8_table  = loop(16, i => PRINCE_SBOX_INVERSE[i] ^ 8).reverse()
const push_sbox_inv_xor_9_table  = loop(16, i => PRINCE_SBOX_INVERSE[i] ^ 9).reverse()
const push_sbox_inv_xor_12_table = loop(16, i => PRINCE_SBOX_INVERSE[i] ^ 12).reverse()
const push_sbox_inv_xor_13_table = loop(16, i => PRINCE_SBOX_INVERSE[i] ^ 13).reverse()
...
const optimize_push_sequence = nestedValues => {
    ...
    for (let i = 0; i < n; i++) {
        ...
        update(i + 1, dp[i] + scriptNumPushCost(values[i]), i, [values[i]]);

        if (i >= 1 && values[i - 1] === values[i]) {
            update(i + 1, dp[i] + 1, i, [OP_DUP]);
        }

        if (i >= 2 && values[i - 2] === values[i]) {
            update(i + 1, dp[i] + 1, i, [OP_OVER]);
        }

        // For table entries that take 2+ bytes to push directly, a nearby OP_PICK
        // often wins. Restricting to depth <= 16 keeps the pick index itself 1 byte.
        for (let depth = 2; depth <= 16 && depth < i; depth++) {
            if (values[i - 1 - depth] === values[i]) {
                update(i + 1, dp[i] + scriptNumPushCost(depth) + 1, i, [depth, OP_PICK]);
            }
        }

        if (i >= 2 && i + 2 <= n &&
            values[i] === values[i - 2] && values[i + 1] === values[i - 1]) {
            update(i + 2, dp[i] + 1, i, [OP_2DUP]);
        }

        if (i >= 3 && i + 3 <= n &&
            values[i] === values[i - 3] && values[i + 1] === values[i - 2] && values[i + 2] === values[i - 1]) {
            update(i + 3, dp[i] + 1, i, [OP_3DUP]);
        }

        if (i >= 4 && i + 2 <= n &&
            values[i] === values[i - 4] && values[i + 1] === values[i - 3]) {
            update(i + 2, dp[i] + 1, i, [OP_2OVER]);
        }
    }
    ...
    for (let i = n; i > 0;) {
        const step = prev[i];
        if (!step) throw new Error(`push optimizer failed at ${i}`);
...
const push_tables_cold_raw = _ => [
    push_and_table_m0,
    push_sbox_inv_xor_5_table,
    push_and_table_m3,
    push_sbox_xor_3_table,
    push_sbox_inv_xor_8_table,
    push_sbox_xor_9_table,
    push_sbox_inv_xor_13_table,
    push_and_table_m2,
    push_sbox_inv_xor_9_table,
    push_sbox_inv_xor_3_table,
    push_and_table_m1,
    push_sbox_xor_4_table,
    push_sbox_xor_8_table,
    push_sbox_inv_xor_12_table,
    push_sbox_xor_14_table,
    push_sbox_xor_2_table,
    push_xor_table,
    push_sbox_inv_xor_7_table,
    push_shift_table,
    push_sbox_inv_table,
]
...
const push_tables_cold = _ => optimize_push_sequence(push_tables_cold_raw())
const push_tables_hot = _ => optimize_push_sequence(push_tables_hot_raw())
...
const op_xor_constant = (constant, scratch = 0) => {
    ...
    return [ XOR_ROW_OFFSET[constant] + scratch + ADDR_XOR_TABLE - 1, OP_ADD, OP_PICK ]
}
...
const op_sbox_inv_xor_constant = (constant, scratch = 0) => {
    switch (constant) {
        case 3:  return [ADDR_SBOX_INV_XOR_3_TABLE  - 1 + scratch, OP_ADD, OP_PICK];
        case 5:  return [ADDR_SBOX_INV_XOR_5_TABLE  - 1 + scratch, OP_ADD, OP_PICK];
        case 7:  return [ADDR_SBOX_INV_XOR_7_TABLE  - 1 + scratch, OP_ADD, OP_PICK];
        case 8:  return [ADDR_SBOX_INV_XOR_8_TABLE  - 1 + scratch, OP_ADD, OP_PICK];
        case 9:  return [ADDR_SBOX_INV_XOR_9_TABLE  - 1 + scratch, OP_ADD, OP_PICK];
        case 12: return [ADDR_SBOX_INV_XOR_12_TABLE - 1 + scratch, OP_ADD, OP_PICK];
        case 13: return [ADDR_SBOX_INV_XOR_13_TABLE - 1 + scratch, OP_ADD, OP_PICK];
        default: return [op_sbox_inv(scratch), op_xor_constant(constant, scratch)];
    }
}
...
const prince_MHatMultiply = (base, useMHat0, pre_action = null, scratch = 0) => {
     ...
     *   and pick the one that minimises encoded bytes. This now
     *   scores both OP_ROLL operand sizes and the later shifted
     *   XOR pointer arithmetic emitted by this M-hat core.      */
    const { perm: bestPerm } =
        bestMHatPermForEnv(ENV, stateIndices, rot, scratch);
...
const prince_m_layer = (pre_action = null) => {
    ...
    for (const groupPerm of PERMS_4) {
        ...
        for (const gi of groupPerm) {
            ...
            const { perm: bestInnerPerm, cost: bestInnerCost } =
                bestMHatPermForEnv(simEnv, sIdx, row.useMHat0 ? 0 : 1, 0);
...
const op_load_key = () => loop(SIZE_KEY, i => [OP_FROMALTSTACK, i + 16, OP_ADD, OP_PICK])
...
const princev2_encrypt = [
    ...
    prince_m_layer(idx_after => {
        ...
        return [
            ...
            op_sbox_inv_xor_constant(RC[6][idx_before]),
            op_copy_key_to_top(idx_before),
            op_xor_shifted()
        ];
    }),
    ...
    loop(4, i => {
        ...
        return [
            ...
            prince_m_layer(idx_after => {
                const idx_before = 15 - PRINCE_SHIFT_INVERSE[15 - idx_after];
                return [
                    op_sbox_inv_xor_constant(RC[r][idx_before]),
    ...
    loop(SIZE_STATE, i => {
        ...
        return [
            ...
            op_sbox_inv_xor_constant(BETA[idx], 0),
